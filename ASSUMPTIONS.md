...
If `-NewUserPassword` is omitted, `Copy-EntraUser` calls the new `New-EntraUserPassword` private helper (the same CSPRNG approach used in the module's own `.EXAMPLE` blocks) to generate one, and sets `ForceChangePasswordNextSignIn = $true` on the created account so the generated password only ever needs to work for a single first sign-in. The generated (or supplied) password is never written to any output stream (Write-Verbose/Warning/Error/Information) — it is unwrapped from its SecureString only at the point of building the Graph API request body. This means the module itself never discloses an auto-generated password back to the operator by default; retrieving or resetting the new user's credential is left to a separate, deliberate flow (Entra's Temporary Access Pass, self-service password reset, or an admin-initiated password reset) rather than the module printing a secret to a stream that might be captured in a transcript or CI log. This is a deliberate security-over-convenience tradeoff, not an oversight.

### Opt-In Password Retrieval via `-PassThru`

`Copy-EntraUser` accepts an opt-in `-PassThru` switch that returns a result object (`NewUserId`, `GeneratedPassword`) instead of producing no output. This exists because the "never discloses it" default above is sometimes genuinely too strict for an operator running the function interactively who wants to see the password they just generated. Rather than reversing the default for every caller (which would leak a secret into every run's console output/transcript unconditionally), `-PassThru` keeps the safe default and makes disclosure an explicit, per-invocation choice. `GeneratedPassword` is populated only when `Copy-EntraUser` itself generated the password (named-parameter path, `-NewUserPassword` omitted); it is `$null` whenever the caller already knows the password (supplied their own `-NewUserPassword`) or used `-NewUser` instead, since there is nothing new to disclose in either case.
