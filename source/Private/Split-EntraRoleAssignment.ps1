# Partitions the template user's directory-role PIM data into roles that
# can be cloned as an ELIGIBLE PIM role assignment and roles that cannot be
# safely cloned by this function at all.
#
# Pure function: no Graph calls, no side effects. Mirrors
# Split-EntraGroupMembership's shape for directory roles instead of groups.
#
...
# is routed to PimRole. A Direct eligibility instance scoped to an
# Administrative Unit is routed to UnsupportedRole.
#
# A Direct active assignment instance with AssignmentType 'Assigned' (a
# permanent assignment never backed by any PIM eligibility) and no
# matching eligibility instance is routed to UnsupportedRole. An active
# assignment instance with AssignmentType 'Activated' is ignored entirely:
# it is just the currently-active form of an eligibility instance already
# accounted for in the PimRole/UnsupportedRole pass above, so counting it
# again would double-report the same role.
#
# Callers must Write-Warning for each entry in UnsupportedRole rather than
# silently dropping them. Returns a hashtable with keys PimRole and
# UnsupportedRole (each System.Object[]).
